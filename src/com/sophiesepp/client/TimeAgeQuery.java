...
public class TimeAgeQuery extends Heatmap implements EntryPoint   {
	...
	public void onModuleLoad() {
		...
		timeagePanel.add(RootPanel.get("heading1"));
	...
	timeagePanel1.addStyleName("timeagePanel1");
	timeagePanel2.addStyleName("timeagePanel2");
	...
	public static String buildQueryTimeAge(){
		...
		if((x==0) &&  (y==0) && (z==0)){
			query="SELECT publication AS publication,(publication-birth) AS age, count(publication-birth) AS counts FROM workspace.composer AS composers JOIN (SELECT publication,personId FROM workspace.work) AS works ON composers.personId=works.personId GROUP BY publication,age"; 
