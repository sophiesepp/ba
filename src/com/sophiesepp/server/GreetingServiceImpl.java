...
GreetingService {
	...
	public void QueryPublicationsPerYear(){
		...
		for(int i=1730; i<2030; i++){
			...
			storeNumberOfPublications(query, year);
...
//  only been executed once to store the sum of publications for each country which is needed for normalization
	public void QueryPublicationsPerCountry(){
		...
		for(int i=0; i<countries.length; i++){
			...
			System.out.println("first query started");
			
			String query = "SELECT composers.country, count(works.publication) FROM (SELECT country AS country, personId AS personId FROM workspace.composer WHERE country='";
			query+=countries[i];
			query+= "') AS composers JOIN workspace.work AS works ON composers.personId=works.personId GROUP BY composers.country"; 
			
			String country = countries[i];
			
			System.out.println(query);
			storeNumberOfPublications(query, country);
...
//  only been executed once to store the sum of publications for each age which is needed for normalization
	public void QueryPublicationsPerAge(){
		...
		for(int i=0; i<100; i++){
			...
			System.out.println("QueryPublicationsPerAge started");
			String age= String.valueOf(i);
			
			String query = "SELECT (works.publication-composers.birth) AS age, count(works.publication-composers.birth) AS counts FROM (SELECT birth AS birth, personId AS personId FROM workspace.composer) AS composers JOIN workspace.work AS works ON composers.personId=works.personId WHERE works.publication-composers.birth=";
			query+=i;
			query+= " GROUP BY age"; 
			
			System.out.println(query);
			storeNumberOfPublications(query, age);
			System.out.println("first query done");
	...
	public List<TimeAge> showQueryTimeAge(String input) throws IllegalArgumentException, IOException {
		...
		{ 
			...
			System.out.println("TOTAL NUMBER OF PUBLICATIONS PER YEAR : "+totalnumberofpublications);
	...
	public List<TimeCountry> showQueryTimeCountry(String input) throws IllegalArgumentException, IOException {
		...
		{ 
			...
			int totalnumberofpublications =  Integer.parseInt(String.valueOf(numberofpublicationsEntity.getProperty("publications")));
			System.out.println("TOTAL NUMBER OF PUBLICATIONS : "+totalnumberofpublications);
	...
	public List<CountriesKey> showQueryCountriesKey(String input) throws IllegalArgumentException, IOException {
		...
		{ 
			...
			int totalnumberofpublications =  Integer.parseInt(String.valueOf(numberofpublicationsEntity.getProperty("publications")));
			System.out.println("TOTAL NUMBER OF PUBLICATIONS IN "+country+":  "+totalnumberofpublications);
